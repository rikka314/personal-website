...
import { ARTICLE_TYPES, BLOG_LANGUAGES, BLOG_PAGE_SIZE, formatBlogDate } from '../lib/blog/runtime'
...
function BlogCard({ article, copy, locale }) {
  return (
    <article className="panel panel-hover p-6 md:p-7">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        <Link className="blog-inline-link" to={`/blog/columns/${article.column}`} viewTransition>
          {article.columnInfo?.name?.[locale] ?? article.column}
        </Link>
        {article.pinned ? <span className="chip">{copy.pinnedLabel}</span> : null}
      </div>

      <h3 className="mt-4 text-2xl font-semibold text-text">{article.title}</h3>
      <p className="mt-4 max-w-3xl text-sm leading-7 text-muted">{article.excerpt}</p>
      ...
      <Link
        className="mt-6 inline-flex items-center gap-2 text-sm font-medium text-accent-hi transition hover:gap-3"
  ...
  return (
    <div className="mt-8 flex flex-wrap items-center justify-between gap-4">
      <p className="text-sm text-muted">
        {locale === 'zh' ? `第 ${currentPage} / ${pageCount} 页` : `Page ${currentPage} of ${pageCount}`}
      </p>
...
export default function BlogHub({ forcedColumnSlug = '' }) {
  ...
  const selectedColumn = forcedColumnSlug || searchParams.get('column') || defaults.column
  ...
  const copy =
    locale === 'zh'
      ? {
          ...
          readArticle: '阅读全文',
          resultSummary: '博客档案',
          sectionCopy: '这里长期记录技术、科研和学习内容，支持搜索、筛选、专栏和分页浏览。',
          sectionEyebrow: forcedColumnSlug ? '专栏' : '博客',
          sectionTitle: forcedColumnSlug ? '专栏内容' : '一个长期维护的技术与学习档案。',
      ...
      : {
          ...
          featuredBody: 'Pinned articles stay at the top and are not repeated in the default list.',
  ...
  useEffect(() => {
    ...
    async function hydrate() {
      ...
      try {
        const [nextIndex, nextSearch] = await Promise.all([loadBlogIndex(), loadBlogSearch()])
        if (!cancelled) {
          setIndex(nextIndex)
  ...
  const columnRecord = useMemo(
    () => index?.taxonomies.columns.find((item) => item.slug === forcedColumnSlug) ?? null,
  ...
  const derivedState = useMemo(() => {
    ...
    const articleMatches = normalizedQuery
      ? new Set(
          searchIndex.entries
            .filter((entry) => entry.kind === 'article' && entry.searchText.includes(normalizedQuery))
            .map((entry) => entry.slug),
        )
      : null
    const projectMatches = normalizedQuery
      ? searchIndex.entries.filter(
          (entry) => entry.kind === 'project' && entry.searchText.includes(normalizedQuery),
        )
      : []
    const hasFilters =
    ...
    const filteredArticles = index.articles.filter((article) => {
      ...
      if (selectedLanguage !== 'all' && article.language !== selectedLanguage) return false
      if (selectedColumn !== 'all' && article.column !== selectedColumn) return false
    ...
    const pageCount = Math.max(1, Math.ceil(listArticles.length / (index.pageSize || BLOG_PAGE_SIZE)))
    ...
    return {
      ...
      visibleArticles: listArticles.slice((safePage - 1) * pageSize, safePage * pageSize),
    }
  }, [
    currentPage,
    deferredQuery,
        ...
        <div className="page-shell">
          <div className="panel p-8">
            ...
            <h2 className="mt-3 text-3xl font-semibold text-text">
              {locale === 'zh' ? '博客内容暂时不可用。' : 'The archive is temporarily unavailable.'}
            </h2>
    ...
    <section className="section-shell">
      <div className="page-shell">
        <div className="section-header">
          ...
          <h2 className="section-title">{columnRecord ? columnRecord.name[locale] : copy.sectionTitle}</h2>
          ...
          <div className="grid gap-5">
              ...
              <section className="panel p-7 md:p-8">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div>
                    ...
                    <h3 className="mt-2 text-2xl font-semibold text-text">{copy.featuredTitle}</h3>
                  </div>
                  <span className="chip">
                    <Sparkles size={14} />
                    {derivedState.featured.length}
                ...
                <p className="mt-4 text-sm leading-7 text-muted">{copy.featuredBody}</p>
                <div className="mt-6 grid gap-5">
                  {derivedState.featured.map((article) => (
                    <BlogCard key={article.slug} article={article} copy={copy} locale={locale} />
                  ))}
                </div>
              </section>
            ) : null}
            ...
            {deferredQuery ? (
              <section className="panel p-7 md:p-8">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div>
                    <p className="tiny-label">{copy.searchLabel}</p>
                    <h3 className="mt-2 text-2xl font-semibold text-text">{copy.matchingProjects}</h3>
                  </div>
                  <span className="chip">{derivedState.projectMatches.length}</span>
                </div>
                <div className="mt-6 grid gap-4">
                  {derivedState.projectMatches.length ? (
                    derivedState.projectMatches.map((entry) => (
                      ...
                      >
                        <p className="tiny-label">{entry.translations[locale].meta}</p>
            ...
            <section className="panel p-7 md:p-8">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  ...
                  <h3 className="mt-2 text-2xl font-semibold text-text">
                    {locale === 'zh' ? `共 ${derivedState.total} 篇文章` : `${derivedState.total} published posts`}
                  </h3>
                </div>
                <p className="max-w-lg text-sm leading-7 text-muted">{copy.searchHint}</p>
              </div>
              ...
              <div className="mt-6 grid gap-5">
                {derivedState.visibleArticles.length ? (
                  ...
                  <div className="soft-surface px-5 py-6">
                    ...
                    <p className="mt-3 text-sm leading-7 text-muted">{copy.emptyBody}</p>
                  </div>
                )}
              </div>
              ...
              <Pager
                ...
                locale={locale}
                onChange={(page) => patchSearchParams(setSearchParams, { page })}
                pageCount={derivedState.pageCount}
              />
            </section>
          </div>

          <aside className="panel h-fit p-7 md:p-8">
            <div className="flex items-center gap-3 rounded-2xl border border-border/80 bg-card/70 px-4 py-3">
              <Search size={18} />
              <input
                className="w-full bg-transparent text-sm text-text outline-none placeholder:text-muted"
                onChange={(event) => patchSearchParams(setSearchParams, { page: 1, q: event.target.value })}
            ...
            <div className="mt-6 grid gap-3">
              <label className="blog-select-shell">
                ...
                <select
                  onChange={(event) => patchSearchParams(setSearchParams, { page: 1, type: event.target.value })}
                  value={selectedType}
                >
              ...
              <label className="blog-select-shell">
                ...
                <select
                  onChange={(event) =>
                    patchSearchParams(setSearchParams, { language: event.target.value, page: 1 })
                  }
              ...
              <label className="blog-select-shell">
                ...
                <select
                  ...
                  onChange={(event) =>
                    patchSearchParams(setSearchParams, { column: event.target.value, page: 1 })
                  }
              ...
              <label className="blog-select-shell">
                ...
                <select
                  onChange={(event) => patchSearchParams(setSearchParams, { page: 1, tag: event.target.value })}
                  value={selectedTag}
                >
            ...
            <button
              ...
              onClick={() => patchSearchParams(setSearchParams, { ...defaults, column: forcedColumnSlug || 'all' })}
              type="button"
            >
              {copy.clearFilters}
            </button>

            <a className="button-primary mt-3 flex w-full justify-center" href={`${ADMIN_PATH_PREFIX}/`}>
